# Cyber Startup: Hardware-Enforced Preemptive Containment Redirection Platform
...
### 1. Multi-Modal Neuro-Symbolic Ingestion (`src/cyberstartup/models/neuro_symbolic.py`)
...
### 2. Continuous-Time Graph Neural Network (`src/cyberstartup/models/ct_gode.py`)
...
Additionally, the GIN threat representation vector $Z_{\text{threat}}$ is mapped to exposed initial entry ports by matching nodes where the index satisfies `src_node` $\pmod 3 = 0$.

### 3. Custom Runge-Kutta 4th Order ODE Solver (`src/cyberstartup/models/ode_solver.py`)
...
### 5. P4 ASIC Controller & TEE Enclave (`src/p4/containment_router.p4`, `src/sgx/sgx_enclave.c`)
...
```bash
PYTHONPATH=src python3 src/cyberstartup/main.py
```
...
Cyber Startup comes with a robust test suite that validates feature correctness, mathematical stability boundaries, and hardware-in-the-loop (HIL) scenario routing.
...
| `CYBERSTARTUP_NO_SUDO` | (Not Set) | **Suppress Sudo**: Disables prompt queries for administrative permissions during test phases or document compilation in Docker. |
| `CYBERSTARTUP_MOCK_TELEMETRY` | (Not Set) | **Telemetry Replay Mode**: Forces the telemetry library to load offline traffic metric replays instead of polling `/sys/fs/bpf/pmu_ringbuf`. Useful for offline incident forensics. |
...
│   │   └── cyberstartup_sgx.manifest    # Gramine LibOS deployment configurations
│   │
│   └── cyberstartup/                    # Core Python modules
